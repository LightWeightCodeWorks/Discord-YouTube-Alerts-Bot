...
- Occassional updates to frequency checks (Well optimized with API already - but check for issues now and then)
- Creating custom messages when the bot shares a new video (This is currently in discussion)
- One more slash command to update the cron - time to check for video activity regularly? Leverage your own API key usage. (will make it easy to update by providing a time/number that converts it into a cron time into the code)
